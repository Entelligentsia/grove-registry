; Sourced from nvim-treesitter (Apache-2.0)
; https://github.com/nvim-treesitter/nvim-treesitter/blob/master/queries/cpp/locals.scm
...
; --- grove additions (see NOTICE) ---
; nvim's cpp locals.scm scopes the function_declarator (parameters) but never
; the function body, and has no pattern for plainly-initialized locals, so body
; locals like `int y = x;` had no enclosing scope. Add block/statement scopes
; and the init_declarator var binding.
[
  (for_statement)
  (if_statement)
  (while_statement)
